...
export async function listIncidents(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const { data, total } = await svc.getIncidents(skip, limit, search);
...
export async function createIncident(req: Request, res: Response, next: NextFunction): Promise<void> {
  try { success(res, await svc.createIncident(req.body as Record<string, unknown>), "Created", 201); } catch (e) { next(e); }
}
...
export async function listTypes(_req: Request, res: Response, next: NextFunction): Promise<void> {
  try { success(res, await svc.getIncidentTypes()); } catch (e) { next(e); }
}

export async function listCategories(_req: Request, res: Response, next: NextFunction): Promise<void> {
  try { success(res, await svc.getIncidentCategories()); } catch (e) { next(e); }
}
